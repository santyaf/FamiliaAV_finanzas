...
| **Partir `App.jsx`** 🟡 en progreso | ✅ `src/lib/` (`finance`, `format`, `notifications`, `amortization`), ✅ `src/ui/` (`theme.js` tokens, `primitives.jsx` Modal/Card/botones…), ✅ `src/sections/` (Conciliacion, Cuentas, Presupuestos). App.jsx: 3.978 → ~3.330 líneas. Faltan: Dashboard, Movimientos, Objetivos, Créditos, Ajustes, QuickCapture, AdminPanel + sus modales. | M |
