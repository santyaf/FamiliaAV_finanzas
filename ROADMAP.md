...
- ~~**Asistente financiero con IA**~~ ✅ — nueva sección Asistente en Gestión: chat que responde con base en un resumen de flujo de caja, presupuestos, objetivos y cuentas (nunca inventa cifras — instrucción explícita en el prompt). Reusa el mismo endpoint `/api/ai-parse` y proveedor de Registro rápido. Falta: seguir a los créditos y a "disponible para gastar hoy" en el resumen que se le manda.
...
- ~~**Unificar Registro rápido + Asistente en una sola pantalla**~~ ✅ — ambos viven ahora en "Asistente" (tarjeta en Gestión + botón flotante), con pestañas "Preguntar"/"Registrar" cuando la persona tiene acceso a los dos (si solo tiene uno, entra directo a ese). El botón flotante de agregar movimiento manual se movió a la barra inferior (antes era "Registro rápido"). El control de acceso sigue siendo independiente por función — solo se unificó la pantalla, no el permiso.
