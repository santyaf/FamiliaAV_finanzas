...
| **Partir `App.jsx`** 🟡 en progreso | ✅ `src/lib/` (finance, format, notifications, amortization), ✅ `src/ui/` (theme, primitives), ✅ `src/sections/` (Conciliacion, Cuentas, Presupuestos, Objetivos, Créditos, Movimientos, QuickCapture — con sus modales). **App.jsx: 3.978 → 1.669 líneas (−56%)**. Faltan: Dashboard (+DonutChart), Ajustes (+InviteModal/CategoryModal/UvrCard/RemindersCard…), AdminPanel, NotificationsPanel, pantallas de auth. | M |
