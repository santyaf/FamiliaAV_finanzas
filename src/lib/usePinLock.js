...
export function usePinLock({ userId, onSignOut, storage = getStorage(), hasher = defaultHasher, startUnlocked = false }) {
  ...
  const unlock = useCallback(async (pin) => {
    ...
    if (result.status === 'signout') { clearPin(storage, userId); setRecord(null); setLocked(false); await onSignOut?.(); return result; }
  ...
  const disable = useCallback(async (pin) => {
    ...
    clearPin(storage, userId); setRecord(null); setLocked(false);
    return result;
  ...
  return { enabled: !!record, locked, timeoutMin: record?.timeoutMin ?? 1, unlock, enable, disable, setTimeoutMin, lockNow: () => { if (recordRef.current) setLocked(true); } };
}
